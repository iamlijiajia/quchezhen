...
//#define kDefaultTintColor   RGB_Color(255, 181, 64)
...
//        self.bgView.layer.borderColor  = kDefaultTintColor.CGColor;
        ...
        UIView *lineV = [[UIView alloc] initWithFrame:CGRectMake(kLeftMargin, self.bgView.bottom + 2, self.bgView.width, 0.5)];
        lineV.backgroundColor = RGB_Color(222, 223, 224);
        ...
        if (items.count >= 2) {
            for (NSInteger i =0; i<items.count; i++) {
                RFSegmentItem *item = [[RFSegmentItem alloc] initWithFrame:CGRectMake(init_x, init_y, itemWidth, itemHeight)
                                                                  ...
                                                                  norColor:RGB_Color(51.0, 51.0, 51.0)
                                                                  selColor:RGB_Color(255.0, 181.0, 64.0)
            ...
            for (NSInteger i = 0; i<items.count-1; i++) {
                init_x += itemWidth;
                UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(init_x, 5, kBorderLineWidth, itemHeight - 5)];
                lineView.backgroundColor = RGB_Color(222, 223, 224);
...
{
    if (!self.itemsArray || !self.itemsArray.count)
    {
        return;
    }
    ...
    else if(currentIndex >= self.itemsArray.count)
    {
        currentIndex = self.itemsArray.count - 1;
    }
    ...
    {
        [_delegate segmentViewSelectIndex:currentIndex];
    }
}
